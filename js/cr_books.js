...
function listBooks(){
    var form = document.forms.namedItem("f_buscar");
    var formData = new FormData(form);
    ...
    xmlhttp.open("POST","../../../private/controller/admin/list_books.php", true); 
    xmlhttp.send(formData);

    return false;
}

function validarIngresoCapitulo(){
    var isbn = document.getElementById("i_isbn").value;
    var aut_codigo = document.getElementById("i_author").value;

    if(isbn.length != 0 && aut_codigo != 0){
        disableCapitulo(false);
    }else{
        disableCapitulo(true);
    }

}

function disableCapitulo(b){
    document.getElementById("i_cap_num").disabled = b;
    document.getElementById("i_cap_title").disabled = b;
    document.getElementById("i_cap_btn").disabled = b;
}